...
pub mod dispatch {
    ...
    impl LiveDispatch {
        ...
        /// CIT-AGENT-3e closed audit F-4: the shipped fleet is now packed + signed (`.cps`),
        /// so the runtime loads + verifies it through `from_archive_verified` with NO override.
        /// Studio no longer touches `CITRATE_ALLOW_UNVERIFIED_CAPSULES` — the bypass is gone.
